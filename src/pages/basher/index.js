...
const SDPage = () => {
  ...
  const printer = (e) => {
    ...
    if (e.target.value == bashCommands[index][0]) {
      setIndex((prevIndex) => prevIndex + 1);
      setInput('');
      setPoints((prevPoints) => prevPoints + Math.floor(1000 * Math.random()));
  ...
  return (
    <Layout pageTitle="Basher">
      <p style={{'white-space': 'pre-wrap'}}>{index - 1 >= 0 && bashCommands[index - 1][2]}</p>
      ...
      <svg>
        ...
        /><text x="20%" y="20" font-family="Arial" font-size="24" fill="black">
          {points}
        </text>
      </svg>
      {displayHelp && <p style={            {width: '200px',
            height: '100px',
            border: '2px solid #000',
            backgroundColor:'green',
            padding: '10px',
            color: 'black'}
}>{bashCommands[index][2]}</p>}
      <button onClick={() => showHelp()}>
        HELP
      </button>
        <textarea onChange={printer} value={input} style={{
          width: '100%',
          height: '200px',
